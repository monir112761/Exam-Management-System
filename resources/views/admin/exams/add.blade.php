...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h3 class="fw-bold mb-0">Add Exam</h3>
        <small class="text-muted">Dashboard / Exams / Add</small>
    </div>
    ...
    <div class="card-body">
        ...
        <form action="{{ route('admin.exams.store') }}" method="POST">
            ...
            <div class="mb-3 mt-3">
                ...
                <textarea name="description" rows="3" class="form-control" placeholder="Enter description">{{ old('description') }}</textarea>
            </div>

            <div class="row g-3">
                <div class="col-md-4">
                    <label class="form-label fw-bold">Duration (Minutes)</label>
                    <input type="number" name="duration" value="{{ old('duration') }}" class="form-control @error('duration') is-invalid @enderror" placeholder="60">
                    ...
                    <div class="border rounded p-2">
                        @foreach($accessTypes as $accessType)
                            <div class="form-check">
                                <input class="form-check-input" type="checkbox" name="access_types[]" value="{{ $accessType->id }}" id="access-{{ $accessType->id }}">
                                <label class="form-check-label" for="access-{{ $accessType->id }}">{{ $accessType->name }}</label>
                            </div>
                        @endforeach
                    </div>
                </div>
            ...
            <div class="mt-4 border rounded p-3">
                <div class="d-flex justify-content-between align-items-center mb-3">
                    ...
                    <span class="badge bg-primary-subtle text-primary-emphasis">Total marks: <strong id="exam-total-marks">0</strong></span>
                </div>
                            ...
                            @foreach($questions as $question)
                                <div class="list-group-item question-mark-row d-flex justify-content-between align-items-center gap-3" data-subject="{{ $question->subject_name ?? 'General' }}" data-unit="{{ $question->exam?->title ?? ($question->subject_name ?? 'General') }}" data-search="{{ strtolower($question->question) }} {{ strtolower($question->subject_name ?? '') }} {{ strtolower($question->exam?->title ?? '') }}">
                                    <div class="form-check flex-grow-1 mb-0">
                                        <input class="form-check-input question-checkbox" data-question-checkbox type="checkbox" name="question_ids[]" value="{{ $question->id }}" id="question-{{ $question->id }}" {{ in_array((string) $question->id, old('question_ids', []), true) ? 'checked' : '' }}>
                                        <label class="form-check-label d-block" for="question-{{ $question->id }}">
